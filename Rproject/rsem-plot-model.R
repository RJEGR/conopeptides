...
dir <- "/Users/cigom/Documents/GitHub/conopeptides/06.Quantification/"

dirs <- list.files(dir, pattern = "_stats", full.names = T)

f <- list.files(dirs, pattern = ".cnt", full.names = T)


read_cntF <- function(f) {
  
  Method <- unlist(strsplit(f, split = "/"))
  Method <- Method[grep("stats", Method)]
  
  token <- unlist(strsplit(basename(f), split = "[.]"))
  
  token <- token[1]
  

  pair <- read.table(file = f, skip = 3, sep = "\t")
...
# argv <- list.files(path = argv,full.names = T)
