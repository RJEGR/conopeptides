...
DEGS %>% dplyr::count(sampleA, sampleB, sampleX, sort = T)

ControlDEGs <- DEGS %>%
  filter(sampleX == "Ctrl") %>%
  distinct(protein_id, sf)
...
ControlDEGs %>% dplyr::count(sf, sort = T)
DEGS %>% dplyr::count(sf, sort = T)


# data 4

CONOPEPDB <- DEGS %>% distinct(protein_id, sf)%>% dplyr::rename("yaxis" = "sf")
...
ddsPCA <- DESeq2::varianceStabilizingTransformation(dds, blind = T)

# datExpr <- assay(dds)


PCA = prcomp(t(assay(ddsPCA)), center = T, scale. = F)
...
plot_heatmap <- HeatmapViz %>%
  ...
  ggh4x::facet_nested(facet ~ Time, scales = "free",space = "free", nest_line = T) +
  # geom_raster() +
...
ggsave(plot_heatmap, filename = 'PUB_heatmap2.png', 
