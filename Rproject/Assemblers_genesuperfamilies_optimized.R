...
recode_col <- structure(c("Full","Multi", "Fragment", "Chimera", "Unannotated", "Unnanotated"), names = recode_col)
...
DB <- conoServerDB |>
  distinct(hits, genesuperfamily, sequence) |> 
  # right_join(DB, by = c("protein_id", "Assembler", "vfold_set")) |>
  right_join(DB, by = c("hits")) 
